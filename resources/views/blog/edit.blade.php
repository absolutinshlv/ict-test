        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('blog.update', $blog) }}">
                ...
                <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-5">
                    <h1 class="text-xl font-bold">Edit blog: {{ $blog->id }}</h1>
                    @if ($errors->any())
                    <div>
                        <ul>
                    ...
                    <div class="mt-10 grid grid-cols-1 gap-x-6 gap-y-8 sm:grid-cols-6">
                            ...
                            <div class="mt-2">
                                ...
                                <div>
                                    <input type="checkbox" value="{{ $category->id }}" id="{{ $category->id }}"
                                        name="category[]" @if ($blog->categories->contains('id', $category->id)) checked
                                    @endif
                                    />
                        ...
                        <div class="col-span-full">
                            ...
                            <div class="mt-2">
                                ...
                                focus:ring-2 focus:ring-inset focus:ring-indigo-600 sm:text-sm sm:leading-6">{{ $blog->body }}</textarea>
                            </div>
                        </div>
                    </div>
                    <button type="submit" class="mt-5 bg-blue-700 text-white p-2 rounded-sm">Update</button>
                </div>
            </form>
        </div>
    </div>
